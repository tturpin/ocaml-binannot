...
/* $Id: owz_parser.mly,v 1.1.2.3 2008/09/09 14:20:46 filliatr Exp $ */
...
let exp_af = { pexp_desc = Pexp_assertfalse ; pexp_loc = Location.none }
let seq_af =  { pexp_desc = Pexp_sequence (exp_af,exp_af);
...
let len_perv = 12 (* Pervasives's length *)
 ...
 */
 ...
 module_expr:
   | mod_longident              { mkmod(Pmod_ident $1) }
   ...
   | STRUCT structure END       { mkmod(Pmod_structure($2)) }
     ...
     /* _correct */
   | STRUCT structure EOF { 
	 add_closing "struct"  "end" (rhs_start 1) (rhs_end 1); 
   ...
   | module_expr LPAREN module_expr RPAREN    { mkmod(Pmod_apply($1, $3)) }
     ...
     /* _correct */
   | module_expr LPAREN module_expr EOF  { 
	 add_closing "("  ")" (rhs_start 2) (rhs_end 2); 
   ...
   | LPAREN module_expr COLON module_type EOF
        { 
	  add_closing "("  ")"  (rhs_start 2) (rhs_end 2); 
	  mkmod(Pmod_constraint($2, $4)) 
	}
   ...
   | LPAREN module_expr RPAREN   { $2 }
     ...
     /* _correct */
   | LPAREN module_expr EOF      
       { 
	 add_closing "("  ")"  (rhs_start 1) (rhs_end 1); 
	 $2 
       } 
 ...
 structure_item:
   ...
   | MODULE UIDENT module_binding
       {
	 let md = $2 in
	 if end_of_file (symbol_end()) then locate_completion md;
	 mkstr(Pstr_module(md, $3)) 
       }
 ...
 module_type:
   ...
   | SIG signature EOF
       { 
	 add_closing "sig"  "end"  (rhs_start 1) (rhs_end 1); 
	 mkmty(Pmty_signature(List.rev $2)) }
 ...
 class_fun_binding:
   ...
   | EOF { print "= assert false"; class_exp }
 ...
 class_expr:
   ...
   | LET rec_flag let_bindings IN class_expr
     ...
     /* VERIF : Que faut-il ecrire dans le buffer ? */

   | LET rec_flag let_bindings EOF
       { 
	 print " in object end";
	 let ce =  
	   { pcl_desc =  Pcl_structure {pcstr_pat = ghpat Ppat_any ; pcstr_fields = []};
	     pcl_loc = Location.none }
	 in
	 mkclass(Pcl_let ($2, List.rev $3, ce)) 
       }
 ...
 class_simple_expr:
   ...
   | class_longident              { mkclass(Pcl_constr($1, [])) }

     /* _begin patch_7  
     *====================*/
   | OBJECT class_structure END   { mkclass(Pcl_structure($2)) }

     /* _correct */
   | OBJECT class_structure EOF   { 
	 add_closing "object"  "end" (rhs_start 1) (rhs_end 1); 
	 mkclass(Pcl_structure($2)) 
       }
       
   | OBJECT class_structure error { unclosed "object" 1 "end" 3 }
   ...
   | LPAREN class_expr COLON class_type RPAREN  { mkclass(Pcl_constraint($2, $4)) }

     /* _correct */
   | LPAREN class_expr COLON class_type EOF     {
	 add_closing "("  ")" (rhs_start 1) (rhs_end 1); 
	 mkclass(Pcl_constraint($2, $4)) 
       }
       
   | LPAREN class_expr COLON class_type error    { unclosed "(" 1 ")" 5 }
   ...
   | LPAREN class_expr RPAREN        { $2 }

       /* _correct */
   | LPAREN class_expr EOF           
       {  
	 add_closing "("  ")" (rhs_start 1) (rhs_end 1); 
	 $2 
       }

   | LPAREN class_expr error         { unclosed "(" 1 ")" 3 }
 ...
 class_self_pattern:
   ...
   | LPAREN pattern COLON core_type RPAREN   { mkpat(Ppat_constraint($2, $4)) }

   | LPAREN pattern COLON core_type EOF      
       { 
	 add_closing "("  ")" (rhs_start 1) (rhs_end 1); 
 ...
 class_signature:
   ...
   | clty_longident                  { mkcty(Pcty_constr ($1, [])) }


     /* _end patch_12  
     *====================*/

   | OBJECT class_sig_body END       { mkcty(Pcty_signature $2) }

     /* _correct */
   | OBJECT class_sig_body EOF       
       { 
	 add_closing "object"  "end" (rhs_start 1) (rhs_end 1); 
	 mkcty(Pcty_signature $2) 
       }
       
   | OBJECT class_sig_body error     { unclosed "object" 1 "end" 3 }
...
class_sig_field:
 ...
 /* _begin patch_x  
   ...
   | QUESTION LPAREN label_let_pattern opt_default EOF
       { 
	 add_closing "("  ")" (rhs_start 2) (rhs_end 2); 
	 ("?" ^ fst $3, $4, snd $3) 
       }
   ...
   | QUESTION label_var                    { ("?" ^ fst $2, None, snd $2) }

     /* _begin patch_13  
     *====================*/

   | OPTLABEL LPAREN let_pattern opt_default RPAREN  { ("?" ^ $1, $4, $3) }

     /* _corrent */
   | OPTLABEL LPAREN let_pattern opt_default EOF     
       { 
 	 add_closing "("  ")" (rhs_start 2) (rhs_end 2); 
	 ("?" ^ $1, $4, $3) }
   ...
   | OPTLABEL pattern_var                          { ("?" ^ $1, None, $2) }

     /* _begin patch_14
     *====================*/

   | TILDE LPAREN label_let_pattern RPAREN       { (fst $3, None, snd $3) }

     /* _correct */
   | TILDE LPAREN label_let_pattern EOF          
       { 
	 add_closing "("  ")" (rhs_start 2) (rhs_end 2); 
	 (fst $3, None, snd $3) 
       }
 ...
 *====================*/
   ...
   | LET rec_flag let_bindings EOF { 
	 untagg_in ();
	 print Tags.tagged_in;
	 mkexp(Pexp_let($2, List.rev $3, exp_af)) }
   ...
   | LET OPEN mod_longident IN seq_expr
     ...
     /* _correct */
   | LET MODULE UIDENT module_binding EOF {
	 print " in assert false";
	 mkexp(Pexp_letmodule($3, $4, exp_af)) }
   ...
   | MATCH seq_expr WITH EOF { 
	 ...
	 let _ = update_match exp AllCs (symbol_end ()) "" in ();
    ...
    /* _correct2 */
   | MATCH seq_expr EOF {
	 if update_match exp AllCs (symbol_end ()) "" then
	   begin   
	     update_cut_pos (symbol_start());
	     let s = Format.sprintf " (*>*)( %s with %s -> %s )(*<*)"
	       (get_expr (symbol_start())(rhs_end 2)) Tags.tagged_any Tags.asf
...
/*   | MATCH seq_expr EOF {
	 let exp = $2 in
	 let closing = Format.sprintf " with %s -> %s" Tags.tagged_any Tags.asf in  
	 update_match exp AllCs (symbol_end ()) closing;
	 mkexp(Pexp_match(exp, [(pat_any,exp_af)]))
       }
*/
     /* _correct3 */
   | MATCH seq_expr WITH opt_bar match_cases EOF  { 
	 let exp = $2 in
	 let cases = List.rev $5 in
	 let closing = Format.sprintf "| %s -> %s" Tags.tagged_any Tags.asf in  
	 let _ = update_match exp (MissCs (patterns cases)) (symbol_end ()) closing
	 in ();
	 mkexp(Pexp_match(exp, cases)) 
       }

     /* _correct4 */
   | MATCH seq_expr WITH opt_bar match_cases_eof EOF { 
	 let exp = $2 in
	 let (br,cases) = $5 in
   ...
   | TRY seq_expr WITH opt_bar match_cases
     ...
     /* _correct_1 */
   | TRY seq_expr WITH EOF { 
	 let exp = $2 in
	 update_try exp AllCs (symbol_end ()) " _ -> assert false";
	 mkexp(Pexp_try($2, [(pat_any,exp_af)])) 
       }

     /* _correct_2 */
   | TRY seq_expr EOF { 
	 let exp = $2 in
	 update_try exp AllCs (symbol_end ()) " with _ -> assert false";
	 mkexp(Pexp_try($2, [(pat_any,exp_af)])) 
       }

     /* _correct_3 */
   | TRY seq_expr WITH opt_bar match_cases EOF { 
	 let exp = $2 in
	 let cases = List.rev $5 in
	 update_try exp (MissCs (patterns cases)) (symbol_end ()) "";
	 mkexp(Pexp_try(exp, cases)) 
       }

     /* _correct_4 */
   | TRY seq_expr WITH opt_bar match_cases_eof EOF { 
	 let exp = $2 in
	 let (br,cases) = $5 in
	 let cs = patterns (List.rev cases) in
   ...
   | TRY seq_expr WITH error           { syntax_error() }
   ...
   | name_tag simple_expr %prec below_SHARP
     ...
     /* _begin patch_19
     *====================*/
   | IF seq_expr THEN expr { mkexp(Pexp_ifthenelse($2, $4, None)) }
   ...
   | IF seq_expr THEN expr ELSE expr
     ...
     /* _correct_1 */
   | IF seq_expr EOF { 
	 print " then assert false else assert false";
	 mkexp(Pexp_ifthenelse($2, exp_af, Some exp_af)) }

     /* _correct_2 */
   | IF seq_expr THEN expr EOF 
     ...
     *====================*/
       ...
       /* _begin patch_20
       *====================*/
   | WHILE seq_expr DO seq_expr DONE   { mkexp(Pexp_while($2, $4)) }

     /* _correct_1 */
   | WHILE seq_expr DO seq_expr EOF { 
	 print " done";
     ...
     /* _correct_2 */
   | WHILE seq_expr EOF { 
	 print " do assert false; done";
	 mkexp(Pexp_while($2, seq_af)) 
       }
   ...
   | FOR val_ident EQUAL seq_expr direction_flag seq_expr DO seq_expr DONE
     ...
     /* _correct_1 */
   | FOR val_ident EQUAL seq_expr direction_flag seq_expr DO seq_expr EOF { 
	 print " done";
	 mkexp(Pexp_for($2, $4, $6, $5, $8)) }

     /* _correct_2 */
   | FOR val_ident EQUAL seq_expr EOF { 
	 print" to (assert false) do assert false; done";
	 mkexp(Pexp_for($2, $4, seq_af, Upto , seq_af)) }

     /* _correct_3 */
   | FOR val_ident EQUAL seq_expr direction_flag seq_expr EOF { 
	 print " do assert false; done";
	 mkexp(Pexp_for($2, $4, $6, $5, seq_af)) }
  ...
  | expr PLUSDOT expr
   ...
   | additive expr %prec prec_unary_plus
     ...
     /* _begin patch_44
     * Already corrected _in patch_43
     *====================*/
   ...
   | simple_expr DOT LPAREN seq_expr RPAREN LESSMINUS expr
       { mkexp(Pexp_apply(ghexp(Pexp_ident(array_function "Array" "set")),
			 ["",$1; "",$4; "",$7])) }
     /*_correct*/
   | simple_expr DOT LPAREN seq_expr EOF
       { 
	 add_closing "("  ") <- assert false" (rhs_start 3) (rhs_end 3); 
	 mkexp(Pexp_apply(ghexp(Pexp_ident(array_function "Array" "set")),
			 ["",$1; "",$4; "",exp_af])) 
       } 
   ...
   | simple_expr DOT LBRACKET seq_expr RBRACKET LESSMINUS expr
       { mkexp(Pexp_apply(ghexp(Pexp_ident(array_function "String" "set")),
			 ["",$1; "",$4; "",$7])) }
     /* correct */
   | simple_expr DOT LBRACKET seq_expr EOF 
       { 
	 add_closing "["  "] <- assert false" (rhs_start 3) (rhs_end 3); 
	 mkexp(Pexp_apply(ghexp(Pexp_ident(array_function "String" "set")),
			 ["",$1; "",$4; "", exp_af])) } 
   ...
   | simple_expr DOT LBRACE expr RBRACE LESSMINUS expr
       ...
       /* correct */
   | simple_expr DOT LBRACE expr EOF
       { 
	 add_closing "{"  "} <- assert false" (rhs_start 3) (rhs_end 3); 
	 bigarray_set $1 $4 exp_af 
       }
   ...
   | label LESSMINUS expr                   { mkexp(Pexp_setinstvar($1, $3)) }
   | ASSERT simple_expr %prec below_SHARP   { mkassert $2 }
   | LAZY simple_expr %prec below_SHARP     { mkexp (Pexp_lazy ($2)) }

     /* _begin patch_26
     *====================*/

   | OBJECT class_structure END      { mkexp (Pexp_object($2)) }
 ...
 simple_expr:
   ...
   | LPAREN seq_expr error       { unclosed "(" 1 ")" 3 }
   ...
   | BEGIN seq_expr END          { reloc_exp $2 }

     /* _correct */  
   | BEGIN seq_expr EOF 
       { 
	 add_closing "begin"  "end" (rhs_start 1) (rhs_end 1); 
	 reloc_exp $2 }

   | BEGIN seq_expr error         { unclosed "begin" 1 "end" 3 }
  ...
  /* peut-on supprimer le EOF et une regle ? */
   | simple_expr DOT label_longident  { mkexp(Pexp_field($1, $3)) }
   ...
   | simple_expr DOT LPAREN seq_expr EOF EOF
       { 
	 add_closing "("  ")" (rhs_start 3) (rhs_end 3); 
	 mkexp(Pexp_apply(ghexp(Pexp_ident(array_function "Array" "get")),
			 ["",$1; "",$4])) }
   ...
   | simple_expr DOT LBRACKET seq_expr EOF EOF
       {
	 add_closing "["  "]" (rhs_start 3) (rhs_end 3); 
	 mkexp(Pexp_apply(ghexp(Pexp_ident(array_function "String" "get")),
			 ["",$1; "",$4])) }
   ...
   | simple_expr DOT LBRACE expr EOF EOF
       { 
	 add_closing "{"  "}" (rhs_start 3) (rhs_end 3); 
	 bigarray_get $1 $4 
       }
   ...
   | LBRACE record_expr RBRACE {
	 let (exten,fields) = $2 
	 in mkexp(Pexp_record(fields, exten)) }
   ...
   | LBRACKETBAR expr_semi_list opt_semi BARRBRACKET
     ...
     /* _correct*/
   | LBRACKETBAR expr_semi_list opt_semi EOF
       {
	 add_closing "[|"  "|]" (rhs_start 1) (rhs_end 1); 
	 mkexp(Pexp_array(List.rev $2)) }
   ...
   | LBRACKET expr_semi_list opt_semi RBRACKET
     ...
     /* _correct*/
   | LBRACKET expr_semi_list opt_semi EOF
       { 
	 add_closing "["  "]" (rhs_start 1) (rhs_end 1); 
	 reloc_exp (mktailexp (List.rev $2)) 
       }
  ...
  | BANG simple_expr
   ...
   | LBRACELESS field_expr_list EOF
       { 
	 add_closing "{<"  ">}" (rhs_start 1) (rhs_end 1); 
	 mkexp(Pexp_override(List.rev $2)) 
       }
 ...
 *====================*/
 match_cases_eof:
   | pattern MINUSGREATER EOF                  { $1, [] }
   | match_cases BAR pattern MINUSGREATER EOF  { $3, $1 }
 ;
 ...
 *====================*/
 match_action:
   ...
   | EOF   { print " -> assert false"; exp_af }

     /* _correct */
   | WHEN seq_expr EOF { 
	 print " (assert false ) -> assert false";
	 mkexp(Pexp_when($2, seq_af)) 
       }
     /* _end patch_35
     *====================*/
 ...
 *====================*/
 record_expr:
   | simple_expr WITH lbl_expr_list opt_semi { (Some $1 , $3)  }
   | lbl_expr_list opt_semi                  { (None , $1) }
   | record_expr_redef                       { $1 }
   | record_expr_def                         { $1 }
 ;
 /* _end patch_45
 ...
 *====================*/
 record_expr_redef:
 /* 01 */ 
   | simple_expr WITH EOF 
       {
	 if rec_is_not_inited () then
	   update_not_inited_redef [] "" [] (rhs_start 1) (rhs_end 1) (symbol_end())
	 else failwith "Parser-1 : This case should not happen";
	 
	 (Some $1, [] )
       } 
       
     /* 02 */
   | simple_expr WITH label_longident EOF 
       {
	 if rec_is_not_inited () then
	   let (md,id) = mod_and_ident () in
	   update_not_inited_redef md id [] (rhs_start 1) (rhs_end 1) (symbol_end())
	 else failwith "Parser-2 : This case should not happen";
	 
	 (Some $1, [] )
       } 
       
     /* 03 */
   | simple_expr WITH lbl_expr_list SEMI EOF 
       {
	 let lbls = $3 in
	 let given = patterns lbls in
	 if rec_is_not_inited () then
	   update_not_inited_redef [] "" given (rhs_start 1) (rhs_end 1) (symbol_end())
	 else failwith "Parser-3 : This case should not happen";
	 
	 (Some $1, lbls ) 
       }
       
     /* 04 */
   | simple_expr WITH lbl_expr_list SEMI label_longident EOF
       { 
	 let lbls = $3 in
	 let given = patterns lbls in
	 if rec_is_not_inited () then
	   let (md,id) = mod_and_ident () in
	   update_not_inited_redef md id given (rhs_start 1) (rhs_end 1) (symbol_end())
	 else failwith "Parser-4 : This case should not happen";
	
	 (Some $1, lbls ) 
       }
       
       
     /* 05 _if blank , 06 _if _not blank */
   | simple_expr EOF 
       {
	 if rec_is_not_inited () then
	   if not (no_space ()) then
	     update_not_inited_redef [] "" [] (rhs_start 1) (rhs_end 1) (symbol_end())
	   else update_value_kind V_record
	 else update_left_imbr ();
	 
	 (Some $1, [] ) 
       }
       
     /* 07 */       
   | simple_expr WITH label_longident EQUAL expr EOF { 
	 if rec_is_not_inited () then
	   update_value_in_redef (symbol_start()) (rhs_end 4) (symbol_end())
	 else 
	   (*TENTATIVE *)
	   update_value_in_redef (symbol_start()) (rhs_end 4) (symbol_end());
     ...
     /* 08 */
   | simple_expr WITH lbl_expr_list SEMI label_longident EQUAL expr EOF{
	 if rec_is_not_inited () then
	   update_value_in_redef (symbol_start()) (rhs_end 6) (symbol_end())
	 else 
	   (*TENTATIVE *)
	   update_value_in_redef (symbol_start()) (rhs_end 6) (symbol_end());
 ...
 *====================*/
 ...
 /* 01  */
  | lbl_expr_list SEMI label_longident EOF {
	let lbls = $1 in
	if rec_is_not_inited () then
	  if no_space () then (
	      let gv = patterns lbls in
	      let (md,id) = mod_and_ident () in
	      update_not_inited_def md id gv (rhs_start 1)(rhs_end 1)(rhs_end 4) 
	    )
	  else failwith "Parser-5 : This case should not happen";
	
	(None, lbls ) 
      }

     /* 02 */
  | lbl_expr_list SEMI EOF 
      {
	let lbls = $1 in
	if rec_is_not_inited () then
	  let gv = patterns lbls in
	  update_not_inited_def [] "" gv (rhs_start 1)(rhs_end 1)(symbol_end())
	else failwith "Parser-6 : This case should not happen";
	
	(None, lbls) 
      }
      
    /* 03 */ 
  | label_longident EQUAL expr EOF {
      
	if rec_is_not_inited () then
	  update_value_in_def (symbol_start()) (rhs_end 2) (symbol_end())
	else 
	  (*TENTATIVE*)
	  update_value_in_def (symbol_start()) (rhs_end 2) (symbol_end());
    ...
    /* 04 */
  | lbl_expr_list SEMI label_longident EQUAL expr EOF { 
	if rec_is_not_inited () then
	  update_value_in_def (symbol_start()) (rhs_end 4) (symbol_end())
	else 
	  (* TENTATIVE *)
	  update_value_in_def (symbol_start()) (rhs_end 4) (symbol_end());
...
lbl_expr_list:
  ...
  | label_longident
      { [$1, exp_of_label $1] }
  | lbl_expr_list SEMI label_longident EQUAL expr { ($3, $5) :: $1 }
...
simple_pattern:
  ...
  | LBRACE lbl_pattern_list record_pattern_end EOF
      { 
	set_rec_inited ();
	update_cut_pos (symbol_start());
	mkpat(Ppat_record(List.rev $2, $3)) 
  ...
  | LBRACKET pattern_semi_list EOF
      { 
	add_closing "["  "]"  (rhs_start 1) (rhs_end 1); 
	reloc_pat (mktailpat (List.rev $2)) 
      }
  ...
  | LBRACKETBAR pattern_semi_list EOF
      { 
	add_closing "[|"  "|]"  (rhs_start 1) (rhs_end 1); 
	mkpat(Ppat_array(List.rev $2)) 
      }
  ...
  | LPAREN pattern error       { unclosed "(" 1 ")" 3 }
  ...
  | LPAREN pattern COLON core_type error  { unclosed "(" 1 ")" 5 }
...
;
  ...
  | label_longident EOF               
    ...
    /* 02 */
  | lbl_pattern_list SEMI label_longident EOF
      { 
	let lbls = $1 in
	if no_space () then
	  if rec_is_not_inited () then
...
constr_longident:
  | mod_longident %prec below_DOT{
	$1
	  (*if end_of_file (symbol_end()) then (
	    let (md,id) = mod_and_ident () in
	    update_value md id (symbol_start()) (symbol_end()) V_exn;
	    );
	    $1
	  *)
      }
...
/* _correct_1 */
  ...
  | mod_longident EOF { 
	let md = $1 in
	update_lbl_longid (flatten md) "" (symbol_start()) (symbol_end());
	md
      }
...
*/
  | mod_longident DOT LIDENT /*maybe EOF*/{
	...
	if can_up_path Class then
	  begin 
	    let st = symbol_start() + 1 in
	    let rg = { b = st ; e = st+ len_af } in
	    let p_c = { pt_kd = Class ;	
			pt_mod = Longident.flatten $1 ; 
			pt_id = $3 ; 
			pt_rg = rg;
			lbrace_pos = -1} in
	    update_path p_c 
