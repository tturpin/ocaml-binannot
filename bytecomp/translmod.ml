...
and transl_structure fields cc rootpath = function
  ...
  | Tstr_recmodule bindings ->
      let ext_fields = List.rev_append (List.map fst3 bindings) fields in
      compile_recmodule
        (fun id modl ->
          transl_module Tcoerce_none (field_path rootpath id) modl)
        bindings
...
let transl_store_structure glob map prims str =
  ...
  | Tstr_recmodule bindings ->
      let ids = List.map fst3 bindings in
      compile_recmodule
        (fun id modl ->
          subst_lambda subst
...
let rec defined_idents items = 
  ...
  | Tstr_primitive(id, descr) -> defined_idents rem
  | Tstr_type decls -> defined_idents rem
  | Tstr_exception(id, decl) -> id :: defined_idents rem
  | Tstr_exn_rebind(id, path) -> id :: defined_idents rem
  | Tstr_module(id, modl) -> id :: defined_idents rem
  | Tstr_recmodule decls -> List.map fst3 decls @ defined_idents rem
  | Tstr_modtype(id, decl) -> defined_idents rem
  | Tstr_open path -> defined_idents rem
  | Tstr_class cl_list ->
...
let transl_toplevel_item item = 
  ...
  | Tstr_recmodule bindings ->
      let idents = List.map fst3 bindings in
      compile_recmodule
