...
let rec type_pat env sp =
  ...
  | Ppat_alias(sq, name) ->
      ...
      rp {
        pat_desc = Tpat_alias(q, TPat_alias id);
        pat_loc = loc;
        pat_type = q.pat_type;
        pat_env = env }
  | Ppat_constant cst ->
  ...
  | Ppat_construct(lid, sarg, explicit_arity) ->
      let (path, constr) = Typetexp.find_constructor env loc lid in
      let sargs =
        match sarg with
          None -> []
  ...
  | Ppat_record(lid_sp_list, closed) ->
      ...
      let type_label_pat (lid, sarg) =
        let (path, label) = Typetexp.find_label env loc lid in
        begin_def ();
  ...
  | Ppat_type lid ->
      ...
      rp {
        pat_desc = Tpat_alias (p, TPat_type path);
        pat_loc = loc;
        pat_type = p.pat_type;
        pat_env = p.pat_env  }
...
let rec is_nonexpansive exp =
  ...
  | Texp_field(exp, _, lbl) -> is_nonexpansive exp
  | Texp_array [] -> true
      ...
      List.for_all
        (fun item -> match item.str_desc with
          ...
          | Tstr_recmodule id_mod_list ->
              List.for_all (fun (_, _, m) -> is_nonexpansive_mod m) id_mod_list
  ...
  | Pexp_record(lid_sexp_list, opt_sexp) ->
      ...
      let opt_exp =
        ...
        | Some sexp, (_, lbl, _) :: _ ->
            ...
            let unify_kept lbl =
              if List.for_all (fun (_, lbl',_) -> lbl'.lbl_pos <> lbl.lbl_pos)
                lbl_exp_list
              then begin
      ...
      let (arg, ty', cty, cty') =
        ...
        | (Some sty, None) ->
            ...
            if !Clflags.principal then begin
                ...
                (type_argument env sarg ty, ty2, Some cty, None)
              end else
              (type_argument env sarg ty, ty, Some cty, None)
        | (None, Some sty') ->
            let (cty', force) =
...
(* prerr_endline "coercion failed"; *)
        ...
        | (Some sty, Some sty') ->
            ...
            if !Clflags.principal then begin
                ...
                generalize_structure ty;
                generalize_structure ty';
                (type_argument env sarg ty, instance ty', Some cty, Some cty')
              end else
              (type_argument env sarg ty, ty', Some cty, Some cty')
      in
      let arg = re {
...
and type_label_exp create env loc ty (lid, sarg) =
  ...
  in
  (path, label, {arg with exp_type = instance arg.exp_type})
  ...
  let rec type_args args omitted ty_fun ty_old sargs more_sargs =
    match expand_head env ty_fun with
      ...
      when (sargs <> [] || more_sargs <> []) && commu_repr com = Cok ->
        ...
        type_args ((l, arg,optional)::args) omitted ty_fun ty_old sargs more_sargs
    | _ ->
        match sargs with
...
and type_expect ?in_function env sexp ty_expected' =
  ...
  | Pexp_sequence(sexp1, sexp2) ->
      ...
      re {
        ...
        exp_env = env }
(*
function ?l:spat=default -> sbody

becomes

function ~l:*opt* ->
  let spat = match *opt* with
     None -> default
   | Some *sth* -> *sth*
  in
  sbody
